...
##'
treeplot.enrichResult <- function(
    x,
    showCategory = 30,
    color = "p.adjust",
    nWords = 4, # removed
    ...
    hc <- stats::hclust(stats::as.dist(1 - termsim2), method = hclust_method)
...
treeplot.compareClusterResult <- function(
    x,
    showCategory = 5,
    color = "p.adjust",
    nWords = 4, # removed
    ...
    cluster.params <- modifyList(default.cluster.params, cluster.params)
    hilight.params <- modifyList(default.hilight.params, hilight.params)
    clusterPanel.params <- modifyList(
        default.clusterPanel.params,
        clusterPanel.params
    )
    offset.params <- modifyList(default.offset.params, offset.params)
    params_list <- list(
    ...
    if (geneClusterPanel == "pie") {
        ID_Cluster_mat$radius <- sqrt(
            p_data$count / sum(p_data$count) * cex_category
        )
        ...
        p <- p +
            ...
            scatterpie::geom_scatterpie_legend(
                ...
                n = legend_n,
                labeller = function(x) {
                    round(sum(p_data$count) * x^2 / cex_category)
                }
            ) +
    ...
    if (geneClusterPanel == "dotplot") {
        ...
        check_installed(
            "ggtreeExtra",
            "for `treeplot()` with ` clusterPanel = 'dotplot'`."
        )
        p <- p +
...
add_cladelab <- function(
    ...
    p <- p +
        ...
        scale_color_manual(values = df$color, guide = 'none')
    if (hilight) {
        p <- p +
            ggtree::geom_hilight(
                data = df,
            ...
            scale_fill_manual(values = df$color, guide = 'none')
    }
...
group_tree <- function(
    ...
    if (inherits(offset_tiplab, "rel")) {
        ...
        if (geneClusterPanel == "pie" || is.null(geneClusterPanel)) {
            ...
            offset_tiplab <- offset_tiplab *
                .5 *
                max(sqrt(d$count / sum(d$count) * cex_category))
